...
def render_factory_planning():
    ...
    with col1:
        recipe_name = st.selectbox(
            ...
            format_func=recipe_display_name_by_id,
        )
    with col2:
...
def _render_production_plan(placed):
    ...
    sel = st.multiselect(
        "Recipes to produce",
        options=options,
        format_func=lambda r: f"{recipe_display_name_by_id(r)} \u2014 {catalog[r]['ws']}",
        key="fs_pp_recipes",
    ...
    for r in sel:
        ...
        with c1:
            m = st.number_input(
                f"Machines \u2014 {recipe_display_name_by_id(r)}", min_value=0, value=1,
        ...
        with c2:
            rate = st.number_input(
                f"Products/hour \u2014 {recipe_display_name_by_id(r)} (measured in game)",
        ...
        if eco:
            ...
            vu, cu = 0.0, 0.0
        rows.append({
            "label": recipe_display_name_by_id(r), "n_machines": int(m), "rate": float(rate),
